...
def get_next_customer_ref(db: Session) -> str:
    from src.backend.models.customer_model import CustomerModel
    count = db.query(CustomerModel).count() + 1
    return generate_ref_id("CUS", count)

def get_next_personnel_ref(db: Session) -> str:
    from src.backend.models.customer_model import PersonnelModel
    count = db.query(PersonnelModel).count() + 1
